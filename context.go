...
// Timestamp adds the current local time to the logger context with the "time" key, formatted using logger.TimeFieldFormat.
// To customize the key name, change logger.TimestampFieldName.
// To customize the time format, change logger.TimeFieldFormat.
//
...
// CallerWithSkipFrameCount adds the file:line of the caller with the logger.CallerFieldName key.
